...
is a small Node process that lives outside `docs/`:

```
server/
  package.json          {"type":"module"}, one dependency: @anthropic-ai/sdk
  serve.mjs             serves ../docs statically and handles POST /api/check
...
This respects `docs/`'s rule (no dependencies, no bundler) because `docs/` does not change
...
independent; `docs/package.json` stays a `"type": "module"` marker.
...
`python3 -m http.server` from `docs/` keeps working. The checkpoint module probes
...
- A static host for `docs/` plus one serverless function (a Cloudflare Worker or a Vercel
...
1. Rules. Role (you grade a reader's restatement of one concept from an explainer), what
   ...
   themselves a miss).
2. The two essays (`design/reference/*.md`) as two more `text` blocks, read from disk at
...
4. The panel's `spine.html` prose, stripped of tags (`<math>` collapses to its text). Read
   from `docs/panels/<slug>/` at boot, keyed by the rubric's `panel`.
...
2. `serve.mjs`: static `docs/` plus `/api/check`, rate limit, verbatim check.
