...
Built. See `design/idea.md` (section "2. Newtonian physics: F = ma") for the beat, and
`design/plan.md` for the pane contract. No pane here has a `.readout`: every number the reader
...
- `left.html` + `left.js`: refresher, Vectors. Two **free** vectors in a pane-local store, not
  ...
  screen. *Departure from `design/idea.md`, which says these are "the same arrows the reviewer
