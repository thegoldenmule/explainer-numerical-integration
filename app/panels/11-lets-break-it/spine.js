...
// against the exact solution, transport, and a two-line readout. The viz must fit beside
// the essay prose at 1280×800: a half plane beside the controls, a strip for the run, and a
// readout that never wraps past two lines.
...
export function mount(root, ctx) {
  ...
  runStage.onDraw(size => {
    ...
    // five short lines: the readout sits in the narrow column beside the plane
    out.set([
      verdict, `  ${factor}\n`,
      `predicted doubling ${secs(r.doublingTime)}\n`,
      `measured doubling ${g ? secs(g.doublingTime) : '…'}\n`,
      `t = ${fmt(cur.t, 2)} s\n`,
      `x ${short(cur.x)} · exact ${short(cur.exact)}`,
    ]);
  });
